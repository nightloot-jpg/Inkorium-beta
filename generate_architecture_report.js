...
- *File Uploads*: Currently, UI components simulate media. Integration with Supabase Storage buckets (policies, resumable uploads) is required for the \`post_images\` to function.
- *Real-time Subscriptions*: \`supabase.channel()\` logic needs to be implemented in \`src/hooks\` to actively listen to the \`notifications\` and \`chat_messages\` tables.
- *Pagination*: TanStack Query infinite query hooks need to be wired up for the feed to handle millions of posts.
...
fs.writeFileSync(path.join(process.cwd(), 'ARCHITECTURE_REPORT.md'), content)
